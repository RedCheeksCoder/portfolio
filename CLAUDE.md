...
Spacing runs off five tokens (`--avatar`, `--action-h`, `--action-gap`, `--social-size`, `--stack-gap`) so the `@media (max-height:700px)` compaction branch overrides five values instead of every rule — same technique as `index.html`'s `@media(max-height:760px)` hero rule. **Measured budget as of the 2026-08-06 restructure: ~650px default, ~529px compact**, against ~730px usable on an iPhone 12/13/14 and ~553px on an iPhone SE — real headroom on both (80px / 24px), a deliberate improvement over the original 5-button layout's ~5px compact margin. **`--action-h` and `--social-size` must not go below 44px** (Apple HIG minimum tap target); tighten elsewhere first.
