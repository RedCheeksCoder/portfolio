...
**31 tools, three data shapes:**
- **27 standard** — `{name, brand, vb, d}`. Monochrome (`var(--text-muted)`) by default, the logo's real brand hex on hover, via a `--brand` CSS custom property set inline per tile.
- **1 multi-color** — Slack (`{name, vb, multi:[{d,fill}]}`). Its real mark is inherently 4-colored with no official single-fill version; per Bryan's explicit choice it's shown in full color always rather than forced monochrome. Its `<svg>` carries a `tool-svg--color` class that the mono/hover CSS rules exclude via `:not()`.
...
- **ChatGPT/OpenAI** — extracted from OpenAI's combined wordmark+glyph SVG (Wikimedia); isolated just the icon-mark path (not the six wordmark letterforms) and computed its own tight bounding box by walking the path commands, rather than reusing the wordmark's much wider viewBox.
- **Slack** — official 4-color mark (Wikimedia), used as-is (see "multi-color" above).
